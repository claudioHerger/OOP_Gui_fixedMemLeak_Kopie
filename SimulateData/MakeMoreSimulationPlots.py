...
def plot_arrays_OOP(x_axis, y_values, x_name, y_name, x_ticks="", x_ticklabels="", title="", file_name="", save_dir="", legend_labels=["legend"]):
    ...
    matplotlib.rcParams.update({'axes.labelsize': 20.0, 'axes.titlesize': 20.0, 'xtick.labelsize':18, 'ytick.labelsize':18, 'legend.fontsize':18})

    colors = ["tab:blue", "tab:orange", "tab:cyan", "tab:olive"]

    fig = matplotlib.figure.Figure(tight_layout=True, figsize=(10,7))
...
def plot_initial_gaussians_from_MultiExcitation_object(simulation_config_file_path="", save_dir=""):
    ...
    plot_arrays_OOP(x_values, initial_gaussians_with_noise.T, "wavelengths", "intensity", title="initial gaussians", file_name=os.path.splitext(os.path.basename(simulation_config_file_path))[0]+"_initial_gaussians", save_dir=save_dir, legend_labels=["component 0", "component 1"])
...
if __name__ == "__main__":
    ...
    simulation_number = 0
    config_file = wavelength_overlap_dir + "overlap_"+str(simulation_number)+".ini"
    print(f'{config_file=}')
